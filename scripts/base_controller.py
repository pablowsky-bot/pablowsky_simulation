...
class baseController(object):
    ...
    def baseCmdVel(self, msg):
        ...
        left_v  = (double) (2 * linear_v - angular_v * self.distance_between_wheels) / (2 * self.wheel_radius);
        right_v = (double) (2 * linear_v + angular_v * self.distance_between_wheels) / (2 * self.wheel_radius);

        # prepare msgs
        left_wheel_msg = Float64()
        right_wheel_msg = Float64()
        left_wheel_msg.data = left_v
        right_wheel_msg.data = right_v
        # publish
